...
// FIXED: Now includes type and choices for MCQ support
// ------------------------------------------------------------
router.get('/:id/random-question', async (req, res) => {
  try {
    ...
    // Build response with all question fields
    const response = {
      ...
      type: question.type || 'open', // 'open' or 'mcq'
    };

    // Include choices if it's an MCQ question
